...
esp32s3dkoctalbuffer::ConfigDef cfg(0);
...
namespace esp32s3dkoctalbuffer {
...
void app_main()
{
    ...
    openmrn_arduino::Esp32WiFiManager wifi_manager(
                                                   ...
                                                   nvs.hostname_prefix());
#endif
    esp32s3dkoctalbuffer::FactoryResetHelper factory_reset_helper;
    ...
    for (int i=0; i < SEQUENCECOUNT; i++)
        LOG(INFO, "[MAIN] Sequence[%d] is %p", i, sequences[i]);
